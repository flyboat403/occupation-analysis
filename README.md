...
# 4. 生成分析报告（需先完成大模型语义分析，生成analysis_data.json）
...
│   ├── international_data.md  # 国际职业数据（Markdown格式）
...
# 根据 ESCO/O*NET 映射查询本地文档，输出Markdown格式供大模型分析使用
# 直接读取原始MD文件，不做解析转换
```
...
# 合并JSON文件（步骤1-4输出）和MD文件（步骤5输出），输出combined_data.md
...
- **步骤3必须等待用户确认**：用 `question` 工具展示确认面板
- **步骤4使用大模型推断**：无需本地文档依赖，直接利用大模型世界知识
- **步骤5输出Markdown格式**：`international_data.md`，直接读取原始MD文件，不做解析转换
- **步骤6输出Markdown格式**：`combined_data.md`，合并JSON和MD文件输出
- **步骤7-14由大模型完成**：语义分析（工作任务分析、典型任务提炼、行动领域划分等）
...
**数据流特征**：
- 步骤5输出：`temp/international_data.md`（335KB，Markdown格式）
- 步骤6输出：`temp/combined_data.md`（338KB，合并JSON+MD文件）
- 步骤7-14由大模型完成：生成`temp/analysis_data.json`含job_tasks/typical_tasks/action_domains等
